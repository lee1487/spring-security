...
AccessDecisionManager
  ...
  - 여러 개의 Voter 들을 가질 수 있으며 Voter들로부터 접근허용, 거부, 보류에 해당하는 각각의 값을 리턴받고 판단 및 결정
  - 최정 접근 거부 시 예외 발생
  ...
  - 접근 결정의 세가지 유형
    ...
    - ConsensusBased
      - 다수표(승인 및 거부)에 의해 최종 결절을 판단한다.
      - 동수일 경우 기본은 접근허가이지만 allowlfEqualGrantedDeniedDesision을 false로
        설정할 경우 접근거부로 결정된다.
    - UnanimousBased:
 ...
 AccessDesisionVoter
   ...
   - Voter가 권한 부여 과정에서 판단하는 자료
     ...
     - FilterInvocation - 요청 정보(antMather("/user"))
